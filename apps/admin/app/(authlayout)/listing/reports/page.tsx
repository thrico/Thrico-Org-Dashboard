...
const Reports = () => {
  ...
  const columns: ColumnsType<ReportData> = [
    ...
    },
    {
      ...
      dataIndex: "reason",
      key: "reason",
      filters: Array.from(new Set(reports.map((item) => item.reason))).map((reason) => ({
        text: reason,
        value: reason,
      })),
      ...
      render: (_, record) => (
        <Space>
          ...
          {record.status === "pending" && (
            <Button type="primary" size="small" onClick={() => showResolveModal(record)}>
              Resolve
            </Button>
          )}
        </Space>
  ...
  return (
    <div>
      ...
      <Card>
        <Tabs activeKey={activeTab} onChange={handleTabChange}>
          <TabPane
            tab={
              <span>
                All Reports <Badge count={reports.length} style={{ marginLeft: 8 }} />
              </span>
            }
            key="all"
          />
          <TabPane
            tab={
              <span>
                Pending <Badge count={pendingCount} style={{ backgroundColor: "#faad14", marginLeft: 8 }} />
              </span>
            }
            key="pending"
          />
          <TabPane
            tab={
              <span>
                Resolved <Badge count={resolvedCount} style={{ backgroundColor: "#52c41a", marginLeft: 8 }} />
              </span>
            }
            key="resolved"
          />
          <TabPane
            tab={
              <span>
                Dismissed <Badge count={dismissedCount} style={{ backgroundColor: "#f5222d", marginLeft: 8 }} />
              </span>
            }
            key="dismissed"
          />
        ...
        <Table columns={columns} dataSource={filteredReports} rowKey="id" pagination={{ pageSize: 10 }} />
      </Card>
      ...
      >
        {selectedReport && (
          <div>
            ...
            <p>
              <strong>Listing:</strong> {selectedReport.listingTitle} (ID: {selectedReport.listingId})
            </p>
            <p>
      ...
      <Modal
        ...
        footer={null}
      >
        <Form form={resolveForm} layout="vertical" onFinish={handleResolve} initialValues={{ action: "resolved" }}>
          ...
          <Form.Item>
            <Space>
              ...
              <Button onClick={() => setIsResolveModalVisible(false)}>Cancel</Button>
            </Space>
          </Form.Item>
        </Form>
      </Modal>
